...
void Vga::draw() {
	...
	for (int y = 0; y < VGA_HEIGHT; ++y) {
		for (int x = 0; x < VGA_WIDTH;
		     x += (sizeof(ensitlm::data_t) * CHAR_BIT)) {

			ensitlm::data_t d;
			initiator.read(
			    address + ((x + (y * VGA_WIDTH)) / CHAR_BIT), d);

			for (unsigned int bit = 0;
			     bit < (sizeof(ensitlm::data_t) * CHAR_BIT);
			     ++bit) {

				Uint16 *bufp =
				    (Uint16 *)screen->pixels + x +
				    (y * screen->pitch / 2) +
				    ((sizeof(ensitlm::data_t) * CHAR_BIT) - 1 -
				     bit);
